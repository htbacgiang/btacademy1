...
const CourseDetail = ({ course, relatedCourses, meta }) => {
  ...
  // Dynamic location schema mapping
  const courseLocations = Array.isArray(course.locations) && course.locations.length > 0
    ? course.locations
    : ["CS1 - Hà Nội"];

  const locationSchema = courseLocations.map(locCode => {
    if (locCode === "CS2 - Thái Nguyên") {
      return {
        "@type": "Place",
  ...
  const offersSchema = course.price ? {
    "@type": "Offer",
    "price": course.price,
    "priceCurrency": "VND"
  } : {
  ...
  const jsonLdData = {
    ...
    "provider": {
      ...
      "sameAs": ["https://www.facebook.com/profile.php?id=61564432965502"]
    },
    "instructor": {
      "@type": "Person",
      "name": "BTV Bích Thủy",
      "jobTitle": "BTV - MC Truyền Hình",
      "worksFor": {
        "@type": "Organization",
        "name": "BT Academy"
      }
    ...
    "timeRequired": `P${course.sessions || 10}W`,
    "courseMode": "onsite",
    "location": locationSchema.length === 1 ? locationSchema[0] : locationSchema,
    "offers": offersSchema,
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 3,
        "name": course.title,
        "item": `https://btacademy.com.vn/khoa-hoc/${slug}`
      }
    ]
  };

  // Person schema (BTV Bích Thủy)
  const personSchema = {
    "@context": "https://schema.org",
    "@type": "Person",
    "name": "BTV Bích Thủy",
  ...
  // FAQ schema
  const faqSchema = course.faq && course.faq.length > 0 ? {
    "@context": "https://schema.org",
    "@type": "FAQPage",
                        ...
                        <div className="grid md:grid-cols-2 gap-4">
                          {course.features.map((feature, index) => (
                            <div key={index} className="flex items-start space-x-3 p-3 bg-green-50 rounded-lg border border-green-100">
                              <div className="w-6 h-6 bg-green-500 text-white rounded-full flex items-center justify-center text-sm flex-shrink-0 mt-0.5">
                                ✓
                              </div>
                      ...
                      <div>
                        ...
                        <ul className="space-y-3">
                          {course.requirements.map((requirement, index) => (
                            <li key={index} className="flex items-start space-x-3 p-3 bg-green-50 rounded-lg border border-green-100">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-8">
                ...
                <button
                  ...
                  className="w-full text-white py-4 px-6 rounded-lg font-semibold transition-all duration-200 transform hover:scale-105 shadow-lg mb-6 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700"
                >
                  Đăng ký ngay
                </button>
                ...
                <div className="p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="space-y-2 text-base">
                    <div className="flex items-center space-x-2 text-gray-600">
                      ...
                      <span>Hotline: 0988 02 7494</span>
                    </div>
                    <div className="flex items-center space-x-2 text-gray-600">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                      </svg>
                      <span>Email:contact@btacademy.com.vn</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-12">
          ...
          {relatedCourses.length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <Link href="/khoa-hoc" className="bg-green-600 text-white px-6 py-3 rounded-lg hover:bg-green-700 transition-colors">
                Xem tất cả khóa học
              </Link>
            </div>
          )}
...
export async function getStaticPaths() {
  try {
    await db.connectDb();
    const courses = await Course.find({ isDeleted: { $ne: true } }).select('slug');
...
export async function getStaticProps(context) {
  ...
  try {
    ...
    const course = await Course.findOne({ slug, isDeleted: { $ne: true } })
      .select('-instructor -instructorRole -category -targetAge -discount -badge -isActive');

    if (!course) {
      return { notFound: true };
    }

    // Fetch related courses (level based, excluding current course)
    ...
    // Fallback if not enough courses of the same level
    if (relatedCourses.length < 3) {
      const extraCourses = await Course.find({
        _id: { $ne: course._id, $nin: relatedCourses.map(c => c._id) },
        isDeleted: { $ne: true }
      })
        .limit(3 - relatedCourses.length)
        .select('title slug image level students sessions rating reviews maKhoaHoc duration schedule location');
      relatedCourses = [...relatedCourses, ...extraCourses];
    }
