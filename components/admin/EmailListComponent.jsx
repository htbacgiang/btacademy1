...
const EmailListComponent = () => {
  ...
  const executeDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/subscription?id=${subIdToDelete}`, {
        method: 'DELETE',
      });
      const data = await response.json();
      ...
      if (data.success) {
        ...
        toast.error(data.message || 'Có lỗi xảy ra khi xóa đăng ký');
      }
    } catch (error) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-x-hidden overflow-y-auto outline-none focus:outline-none">
          ...
          <div className="relative w-full max-w-md mx-auto my-6 z-50 px-4">
            <div className="relative flex flex-col w-full bg-white border-0 rounded-2xl shadow-2xl outline-none focus:outline-none transform transition-all scale-100">
              {/* Header */}
              <div className="flex items-start justify-between p-5 border-b border-gray-150 rounded-t-2xl">
              ...
              <div className="relative p-6 flex-auto">
                <p className="text-gray-600 text-base leading-relaxed">
                  Bạn có chắc chắn muốn xóa đăng ký này khỏi danh sách không? Hành động này không thể hoàn tác.
                </p>
              </div>

              {/* Footer */}
              <div className="flex items-center justify-end p-6 border-t border-gray-150 rounded-b-2xl gap-3">
                <button
                  className="px-5 py-2.5 bg-gray-100 hover:bg-gray-250 text-gray-700 font-medium rounded-xl text-sm transition duration-200"
                  type="button"
                  onClick={() => setIsDeleteModalOpen(false)}
                >
                  Hủy bỏ
                ...
                <button
                  className="px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white font-medium rounded-xl text-sm shadow-md hover:shadow-lg transition duration-200"
                  type="button"
