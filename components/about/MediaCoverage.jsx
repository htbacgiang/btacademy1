  ...
  const getTypeIcon = (type) => {
    switch (type.toLowerCase()) {
      ...
      case 'phỏng vấn độc quyền':
        return <FaQuoteLeft className="text-green-600" />;
      default:
    ...
    <div className="bg-gradient-to-br from-gray-50 via-white to-gray-50 py-16">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {mediaData.map((item) => (
            <div key={item.id} className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 overflow-hidden border border-gray-100">
              {/* Image */}
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute top-3 left-3">
                  <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium border border-green-200 ${getTypeColor(item.type)}`}>
        ...
        <div className="text-center mt-12">
          <div className="bg-gradient-to-r from-green-600 to-emerald-600 rounded-2xl p-8 text-white">
            ...
            <button 
              ...
              className="inline-flex items-center gap-3 bg-white text-green-600 px-8 py-3 rounded-full font-semibold hover:bg-green-50 transition-colors"
            >
              Đăng ký ngay
              <FaExternalLinkAlt />
            </button>
        ...
        <div className="absolute top-40 right-20 w-3 h-3 bg-emerald-500 rounded-full animate-ping delay-500 opacity-30"></div>
        <div className="absolute bottom-32 left-1/4 w-1 h-1 bg-green-600 rounded-full animate-ping delay-1000 opacity-30"></div>
      </div>
